...
function Get-BaseDirectory {
    if ($PSScriptRoot -and (Test-Path $PSScriptRoot)) {
        # Cas ou le script est lance directement : on utilise le dossier du .ps1
        return $PSScriptRoot
    }
    # Cas ou le script est execute via le launcher (ScriptBlock) :
    # on se base sur le dossier courant, deja positionne par le .bat / launcher
...
function Show-AppHeader {
    ...
    if ($StepSubtitle) {
        Center-Write $StepSubtitle ([ConsoleColor]::Gray)
    }
    Write-Host ""
}
...
function Show-BoxCentered {
    ...
    if ($Title) {
        $t = $Title
        if ($t.Length -gt $innerWidth) {
            $t = $t.Substring(0, $innerWidth)
        }
...
# ---------------------------------------------------------------------
#  ETAPE 2/4 - IDENTITE UTILISATEUR
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
#  ETAPE 3/4 - CHOIX TEAM
# ---------------------------------------------------------------------

Show-AppHeader "ETAPE 3/4 - TEAM / SERVICE" "Selectionnez votre equipe / Select your team"
...
try {
    ...
    Show-BoxCentered -Title "CONFIG INVALIDE / INVALID CONFIG" -Lines @(
        ...
        "Details in log file: inventory_error.log"
    )
    Wait-EnterCentered "Appuyez sur ENTREE pour fermer / Press ENTER to close"
    exit 1
}
